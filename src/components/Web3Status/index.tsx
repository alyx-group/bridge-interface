...
function Web3StatusInner() {
  ...
  // console.log('sortedRecentTransactions', sortedRecentTransactions[0]?.receipt)
  ...
  if (account) {
    return (
      <Web3StatusConnected id="web3-status-connected" onClick={toggleWalletModal} pending={hasPendingTransactions}>
        {hasPendingTransactions ? (
          ...
          <>
            {hasSocks ? <Sock /> : null}
            <AccountText fontSize={isMobile? "14px": "14px"}>{ENSName || shortenAddress(account)}</AccountText>
          </>
        )}
    ...
    return (
      <Web3StatusConnect id="connect-wallet" onClick={toggleWalletModal} faded={!account}>
        <Text>
          <AccountText fontSize={isMobile? "1rem": "1rem"}>Connect</AccountText>
        </Text>
      </Web3StatusConnect>
    )
  }
