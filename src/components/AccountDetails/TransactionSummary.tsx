...
function BridgeSwapSummary({ info }: { info: BridgeSwapTransactoinInfo  }) {
  return (
    <Trans>
      Bridge Transfer exactly{' '}
      <FormattedCurrencyAmountManaged
        rawAmount={info.inputCurrencyAmountRaw??''}
        currencyId={info.inputCurrencyId??''}
        sigFigs={6}
...
export function TransactionSummary({ info }: { info: TransactionInfo }) {
  switch (info.type) {
    ...
    case TransactionType.BRIDGE_SWAP:
      return <BridgeSwapSummary info={info} />
      // return <SwapSummary info={info} />
