...
export function useCurrencyBalances(
  ...
  const containsETH: boolean = useMemo(() => currencies?.some((currency) => currency?.isNative || currency?.wrapped.address ==='0x0000000000000000000000000000000000000000') ?? false, [currencies])
  ...
  return useMemo(
    () =>
      currencies?.map((currency) => {
        ...
        if (currency.wrapped.address ==='0x0000000000000000000000000000000000000000'){
