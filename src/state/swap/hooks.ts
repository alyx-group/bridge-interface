...
export function useSwapActionHandlers(): {
  ...
  onSwitchTargetChain: (chain: string) => void
  onChangeRecipient: (recipient: string | null) => void
} {
  const dispatch = useAppDispatch()
  const { chainId, library } = useActiveWeb3React()
  const bridgeContract = useBridgeContract(chainId)
  const targetChain = useAppSelector(state => state.swap.targetChain)

  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      // console.log("useSwapActionHandlers->onCurrencySelection->", field, currency)
      dispatch(
        selectCurrency({
          field,
    ...
    [dispatch, bridgeContract, targetChain]
  )
  ...
  const onAddressInput = useCallback(
    (address: string) => {
      dispatch(inputTargetAddress({ address }))
    },
    [dispatch]
  )
...
export function useDerivedSwapInfo(
  ...
  if (!targetChain) {
    inputError = inputError ?? t`Select target chain`
  }
  ...
  if (typedValue) {
    ...
    if (buyNative) {
      if (minDeposit) {
        ...
        inputError = inputError ?? t`Fetching min deposit`
      }
    }
  ...
  if (!targetAddress) {
    inputError = inputError ?? t`Enter receiving address`
  }
